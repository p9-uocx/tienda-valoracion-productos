...
exports.RoleModel = new (class RoleModel extends Mysql {
  getRolById(id) {
    ...
    return this.query(`SELECT * FROM Roles WHERE id_roles=${id}`).then(data =>
      data.map(parseBoolean),
    );
  }

  getAllRoles() {
    return this.query('SELECT * FROM Roles').then(data => data.map(parseBoolean));
  }
  ...
  updateRol(
    ...
    return this.query(
      `UPDATE Roles SET ` +
        `name='${name}',edit_product=${edit_product},create_product=${create_product},delete_product=${delete_product},edit_review=${edit_review},create_review=${create_review},delete_review=${delete_review},dashboard_access=${dashboard_access},edit_category=${edit_category},create_category=${create_category},delete_category=${delete_category},edit_user=${edit_user},create_user=${create_user},delete_user=${delete_user},edit_role=${edit_role},create_role=${create_role},delete_role=${delete_role} ` +
        `WHERE id_roles=${id}`,
    );
  }
})();
